...
class AgentLoop:
            ...
            try:
                ...
                if budget_decision.should_compact:
                    ...
                    if self.compaction_policy is not None:
                        ...
                        compaction = await self.compaction_policy.compact(context, target_tokens)
                ...
                for item in response.items:
                    ...
                    if isinstance(item, AssistantMessage):
                        ...
                        await self._handle_final_answer(state, trace, item)
                    else:
                        raise TypeError(f"unsupported_response_item: {type(item).__name__}")

                for call in pending_tool_calls:
                    state.messages.append(
                        await self._execute_tool_call(state, trace, call)
                    )
                ...
                state.error = "cancelled_by_caller"
                self._event(
                    state,
                    trace,
                ...
                self._event(state, trace, EventType.RUNTIME_ERROR, error_data)
    ...
    async def _handle_final_answer(
        ...
        self._event(
            state,
            trace,
            EventType.TASK_VERIFICATION_STARTED,
            {"candidate_answer": answer.content},
        )
        result = await self.task_verifier.verify(state.task, answer.content, state)
        ...
        self._event(
            state,
            trace,
            EventType.TASK_VERIFICATION_RESULT,
            result.to_dict(),
        )
        if result.passed:
            state.final_answer = answer.content
        ...
        state.messages.append(Message(role="assistant", content=answer.content))
        state.messages.append(
            Message(
                role="verification",
                name="task_verifier",
    ...
    async def _execute_tool_call(
        ...
        result_message = Message(
            ...
            content=rendered_output.content,
        )
        self._event(
            state,
            trace,
